...
-export([twitter_example/0, spawnImportant/2,  isthere/3, check_filter/2, split_transformer/0, decorate_with_id/1, twitter_print_pipeline/3, twitter_producer/2, get_account_keys/1]).
...
%%Not sure everything is checked since there might be more heads
check_filter(Tweet, Hashtag) ->
isthere(Tweet, Hashtag, string:str(hd(Tweet),hd(Hashtag))).

isthere(Tweet, Hashtag, B) when B > 0 ->
	io:format("~s AddCount ", [Hashtag]), 
        twitterminer_riak:count(Hashtag);
isthere(Tweet, Hashtag, B) ->ok. %% io:format("No").
...
twitter_example() ->
	{_,Movies}=twitterminer_riak:bucketRequest("HashtagsTest1"),
...
%URL = "https://stream.twitter.com/1.1/search/tweets.json",
  ...
  % Run our pipeline
  P = twitterminer_pipeline:build_link(twitter_print_pipeline(URL, Keys,Movies)),
...
%% prints tweets.
twitter_print_pipeline(URL, Keys,Movies) ->
  ...
  [
    twitterminer_pipeline:consumer(
     fun(Msg, N) -> my_print(Msg,Movies), N+1 end, 0),
    %% fun(Msg, N) -> my_filter(Msg, "the"), N+1 end, 0),
...
receive_tweets({init, URL, Keys}) ->
 ...
 %{track,"paris"}
 %{language,en}
  SignedParams = oauth:sign("GET", URL, [{delimited, length}, {language, en}, 
  ...
  case T of
    {parsed_tweet, L, B, _} ->
      ...
      case extract(<<"text">>, L) of
        {found, TT} -> Tweet = io_lib:format("~s", [TT]),
	%%check_filter(Tweet,"this");
	%%spawn(twitterminer_source,check_filter,[Tweet,"mug"]);
   	spawnImportant(Tweet,Movies);
    %%io:format("tweet: ~ts~n", [TT]);
        not_found -> ok
...
%%RHP = twitterminer_riak:get_riak_hostport(riak1),
%{ok,R} = riakc_pb_socket:start(RHP#hostpost.host, RHP#hostport.port),

%riakc_pb_socket:list_keys(R,<<"HashtagTest1">>).

%Command = "GET http://129.16.155.12:10018/buckets/HashtagsTest1/keys?keys=true", X = os:cmd(Command),X.


spawnImportant(Tweet, [X|Y]) -> XX = io_lib:format("~s", [X]),
				spawn(twitterminer_source,check_filter,[Tweet,XX]),
			 spawnImportant(Tweet,Y);
spawnImportant(Tweet, X) -> ok. 
